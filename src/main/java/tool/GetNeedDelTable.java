...
public class GetNeedDelTable {
    ...
    public static void m() {
        String allTables = "admin, admin_auth_assignment, admin_auth_item, admin_auth_item_child, branch, cdouyin_order2021, cdouyin_order2022, channel, channel_order_talent, channel_talent, group, member, member_add_store, member_mission, member_talent, order_talent, partner, product, product_category, product_spec, shop, shop_share, team, xingxuan_category_relation";
        String tables = "member,member_talent,channel_talent,channel,branch,group,team,shop,partner,order_talent";
        List<String> collect = Arrays.stream(allTables.split(", ")).collect(Collectors.toList());
        List<String> use = Arrays.stream(tables.split(",")).collect(Collectors.toList());
        collect.removeAll(use);
